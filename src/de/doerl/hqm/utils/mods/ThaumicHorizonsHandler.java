...
class ThaumicHorizonsHandler extends AHandler {
	...
	boolean isBlock( String name) {
		return name.startsWith( "assets/thaumichorizons/textures/blocks");
	}

	@Override
	boolean isItem( String name) {
		return name.startsWith( "assets/thaumichorizons/textures/items");
	}

	@Override
	boolean isModFile( String name) {
		return name.startsWith( "thaumichorizons");
	}
}
