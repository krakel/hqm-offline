...
class HarvestcraftHandler extends AHandler {
	...
	boolean isBlock( String name) {
		return name.startsWith( "assets/harvestcraft/textures/blocks");
	}

	@Override
	boolean isItem( String name) {
		return name.startsWith( "assets/harvestcraft/textures/items");
	}

	@Override
	boolean isModFile( String name) {
		return name.startsWith( "Pam's HarvestCraft");
	}
}
