...
class GendustryHandler extends AHandler {
	...
	boolean isBlock( String name) {
		return name.startsWith( "assets/gendustry/textures/blocks");
	}

	@Override
	boolean isItem( String name) {
		return name.startsWith( "assets/gendustry/textures/items");
	}

	@Override
	boolean isModFile( String name) {
		return name.startsWith( "gendustry");
	}
}
