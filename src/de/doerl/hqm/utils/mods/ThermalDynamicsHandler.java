...
class ThermalDynamicsHandler extends AHandler {
	...
	boolean isBlock( String name) {
		return name.startsWith( "assets/thermaldynamics/textures/blocks");
	}

	@Override
	boolean isItem( String name) {
		return name.startsWith( "assets/thermaldynamics/textures/items");
	}

	@Override
	boolean isModFile( String name) {
		return name.startsWith( "ThermalDynamics");
	}
}
